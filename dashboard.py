...
def load_data():
    if not os.path.exists(LOG_PATH):
        st.error("❌ trade_setups.csv not found at expected path.")
        return pd.DataFrame()
    ...
    st.write("✅ Loaded trades:", df.shape)
...
def display_trade_table(df):
    st.write("### 📋 Trade Log")
    st.dataframe(df[[
        "timestamp", "symbol", "direction", "score", "trade_type", "result", "entry", "sl", "tp1", "tp2"
...
def display_indicator_details(df):
    ...
    try:
        indicators = json.loads(latest["used_indicators"]) if pd.notna(latest["used_indicators"]) else []
    ...
    try:
        scores = json.loads(latest["indicator_scores"]) if pd.notna(latest["indicator_scores"]) else {}
...
def main():
    ...
    st.title("📈 Crypto Bot Trade Summary")
    ...
    # Disable filters until confirmed working
    # df = filter_data(df)
